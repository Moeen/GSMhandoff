...
public class DigitsEditText extends EditText {
	...
	public DigitsEditText(Context context, AttributeSet attrs) {
		super(context, attrs);
		/*
		 * if(Compatibility.isCompatible(Build.VERSION_CODES.JELLY_BEAN)) { try
		 * { showSoftInputOnFocus =
		 * UtilityWrapper.safelyGetSuperclassMethod(getClass(),
		 * "setShowSoftInputOnFocus", boolean.class); }catch(RuntimeException e)
		 * { Log.e(THIS_FILE, "Cannot find show soft input on focus method ",
		 * e); } }
		 */
			...
			if (isDigit) {
				...
				setTextSize(
						...
						getContext().getResources().getDimension(
								R.dimen.dialpad_digits_text_size));
			} else {
	...
	/*
	 * // @Override public boolean isTextSelectable() { if(isDigit == null) {
	 * return false; } return !isDigit; }
	 */
	...
	/*
	 * @Override public boolean requestFocus(int direction, Rect
	 * previouslyFocusedRect) { boolean ret = false; if(!isDigit) { ret =
	 * super.requestFocus(direction, previouslyFocusedRect); }
	 * applyKeyboardShowHide(false); return ret; }
	 */
